            ...
            try:
                r = yt.playlists().insert(
                    ...
                    body={
                        "snippet": {
                            "title": nome,
                            "description": f"Orações ao vivo e VODs — {nome}",
                            "defaultLanguage": "pt",
                        },
...
def _renomear_vod_e_classificar(bid: str):
    ...
    weekday = now.weekday()
    titulo_base = re.sub(r"[🔴🟢🔵]|— Oração Poderosa|\s*AO VIVO", "", TITULOS_LIVE[weekday]).strip()
...
def loop_transmissor():
    ...
    if MODO_PERMANENTE:
        ...
        try:
            yt = get_youtube()
            log.info("YouTube API PT OK — broadcast novo a cada ciclo de 12h.")
        except Exception as e:
            log.error(f"YouTube API PT indisponível ({e}) — SEM auto-broadcast!")

        while not _ev_parar.is_set():
            ciclo += 1
            log.info(f"Transmissor PT — ciclo {ciclo} de 12h")
            if yt is None:
                try:
                    yt = get_youtube()
                    log.info(f"YouTube API PT re-inicializado no ciclo {ciclo}.")
                except Exception as _reinit:
                    log.warning(f"YouTube API PT re-init falhou no ciclo {ciclo}: {_reinit}")
            proc_h = None
                    ...
                    if not bid_h:
                        for _tentativa_bc in range(5):
                            try:
                                ...
                                log.warning(f"criar_broadcast PT: tentativa {_tentativa_bc+1}/5 ERRO: {_ebc}")
                                bid_h = None
                            if bid_h:
                                break
                            log.warning(f"criar_broadcast PT: tentativa {_tentativa_bc+1}/5 sem ID — aguardando 30s")
                            _ev_parar.wait(timeout=30)
                        if not bid_h:
                            log.error("criar_broadcast PT: todas as 5 tentativas falharam — watchdog tentará a cada 2min")
                    else:
                        try:
            ...
            ultimo_refresh_rtmp = ciclo_start - 1800  # offset -30min vs ES: PT dispara 11.5h após start, ES dispara 12h → nunca simultaneamente
                ...
                while not _ev_parar.is_set():
                    ...
                    if proc_h.poll() is not None:
                        ...
                        if uptime < 30:
                            ...
                            log.warning(f"FFmpeg H PT encerrou em {uptime:.0f}s (rc={rc}) — falha RTMP #{_falhas_rtmp}, aguardando {espera}s")
                            _ev_parar.wait(timeout=espera)
                            if _falhas_rtmp >= 4 and yt and bid_h:
                                log.warning("FFmpeg H PT: 4 falhas RTMP consecutivas — forçando broadcast novo")
                            ...
                            _falhas_rtmp = 0
                            log.warning(f"FFmpeg H PT encerrou (rc={rc}) — reconstruindo e reiniciando")
                    ...
                    if yt and (time.time() - ultimo_check_bc) >= 120:
                        ultimo_check_bc = time.time()
                        if not bid_h:
                            log.warning("Watchdog PT: bid_h=None — tentando criar broadcast agora")
                            try:
                                bid_h = criar_broadcast_permanente(yt)
                            except Exception as _ewdg:
                                log.warning(f"Watchdog PT: criar_broadcast falhou: {_ewdg}")
                                bid_h = None
                            if bid_h:
                                with _lock:
                                    _estado["live_id_h"] = bid_h
                            ...
                            for _tentativa in range(3):
                                try:
                                    ...
                                    log.warning(f"watchdog broadcast PT (tentativa {_tentativa+1}/3): {e}")
                                    if _tentativa < 2:
                                        time.sleep(10)
                            if st in ("complete", "revoked"):
                                ...
                                try:
                                    ...
                                    log.warning(f"Watchdog PT (revoked): criar_broadcast falhou: {_erv}")
                                    bid_h = None
                                if bid_h:
                                    with _lock:
                                        _estado["live_id_h"] = bid_h
            ...
            if _ev_parar.is_set():
                break
            log.info("Ciclo PT 12h concluído — aguardando 60s para YouTube salvar VOD...")
...
_CHAT_GEMINI_KEYS_PT = [k for k in [
    os.environ.get("GEMINI_KEY_LIVE_CONTENT_1_PT", ""),
    os.environ.get("GEMINI_KEY_LIVE_CONTENT_2_PT", ""),
] if k]

def _eh_mensagem_respondivel_pt(texto: str) -> bool:
    import re
    if len(texto) < 6:
        return False
...
def _gerar_resposta_chat_pt(autor: str, texto: str) -> str | None:
    ...
    if not chaves:
        return None
    t = texto.lower()
    if any(p in t for p in ["única", "único", "sozinha", "sozinho", "só eu",
    ...
    for chave in chaves:
        try:
            ...
            if "429" in str(e) and chave != chaves[-1]:
                continue
            log.warning(f"chat_gemini PT: {e}")
            return None
    return None
    ...
    while not _ev_parar.is_set():
        ...
        try:
            ...
            for item in resp.get("items", []):
                ...
                if not _eh_mensagem_respondivel_pt(texto):
                    continue
                resposta = _gerar_resposta_chat_pt(autor, texto)
                if resposta:
                    try:
                        ...
                        log.info(f"Chat PT respondido: @{autor} → {resposta[:60]}...")
                        respondeu = True
                        respostas_hora += 1
                    except Exception as e:
                        log.warning(f"Chat PT insert: {e}")

            if len(ids_vistos) > 2000:
                ids_vistos = set(list(ids_vistos)[-500:])
        except Exception as e:
            log.warning(f"loop_respostas_chat PT: {e}")
